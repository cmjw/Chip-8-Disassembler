...
void Disassembler::disassemble(const std::vector<uint8_t>& buffer) {
    if (buffer.size() % 2 != 0) {
        std::cerr << "Error: Program size must be even (aligned to 2-byte opcodes)." << std::endl;
        return;
    }

    for (size_t i = 0; i < buffer.size(); i += 2) {
        uint16_t opcode = (buffer[i] << 8) | buffer[i + 1];
        std::string instruction = decodeOpcode(opcode);
...
std::string Disassembler::decodeOpcode(uint16_t opcode) {
    ...
    switch (opcode & 0xF000) {
        ...
        case 0x1000: return "JP " + std::to_string(nnn);
